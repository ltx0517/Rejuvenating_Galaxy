...
drpall = fits.open("/Users/txl/Desktop/MaNGA/Data/drpall-v3_1_1.fits")[1].data
...
# =====================================
def get_masked(plateifu, emline, snr = 3, path = apparatus['Mac_maps_path']): # snr = 5 default
...
def HaEW(plateifu, path = apparatus['Mac_maps_path']):
    filename = path + 'manga-' + plateifu + '-MAPS-HYB10-MILESHC-MASTARSSP.fits.gz'
...
def sigma(plateifu, path = apparatus['Mac_pipe_path']):
    ...
    hdu_m  = fits.open(path + 'manga-' + plateifu + '.Pipe3D.cube.fits.gz')
    mass = hdu_m['SSP'].data[19] # stellar mass density per pixel with dust correction
...
def spaxel_area_pc_and_redshift(plateifu):
    ...
    ind  = np.where(drpall['plateifu'] == plateifu)
    z    = drpall['nsa_z'][ind][0] # Redshift of the galaxy
...
def kewley_nii_nonsf_mask(y, x1):
    curve = 0.61/(x1 - 0.05) + 1.3
    mask  = y > curve
    return mask
...
def BPT_sf_mask(gal, snr = 3):
    ...
    mask_bad = bad_nii | bad_sii | bad_oi
